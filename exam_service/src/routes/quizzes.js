...
const { generateMcqs, OllamaMcqError } = require('../services/ollamaMcq');

const router = express.Router();
const OPTIONS = ['A', 'B', 'C', 'D'];

function buildContext(rows) {
  const maximumCharacters = Number(process.env.OLLAMA_EXAM_CONTEXT_CHARS || 40000);
  const seen = new Set();
  const sections = [];
...
router.post('/generate', requireTeacher, async (req, res) => {
  ...
  try {
    ...
    if (error instanceof OllamaMcqError) {
